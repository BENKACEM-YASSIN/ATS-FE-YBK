...
export class PdfPreviewComponent implements OnInit, AfterViewInit {
  ...
  paginate() {
    ...
    const PAGE_MARGIN_BOTTOM = 32; // Small bottom buffer (~8mm) for natural page margin
    const PAGE_TOP_SPACER_PX = 0; // Spacing handled via print padding
    const MAX_CONTENT_HEIGHT = A4_HEIGHT_PX - PAGE_MARGIN_BOTTOM;
    const OVERFLOW_EPS = 2; // Allow tiny rounding differences

    // Safety break to prevent infinite loops
    let pageCount = 1;
    const MAX_PAGES = 10;

    // We loop and split only when actual content overflows the page height.
    while (pageCount < MAX_PAGES) {
        ...
        if (isTwoColumn) {
            ...
            const leftCol = leftSelector ? currentPage.querySelector(leftSelector) as HTMLElement : null;
            const rightCol = rightSelector ? currentPage.querySelector(rightSelector) as HTMLElement : null;
            ...
            const leftSections = Array.from(leftCol.querySelectorAll(':scope > .section-block')) as HTMLElement[];
            const rightSections = Array.from(rightCol.querySelectorAll(':scope > .section-block')) as HTMLElement[];
            ...
            const nextLeftCol = leftSelector ? nextPage.querySelector(leftSelector) as HTMLElement : null;
            const nextRightCol = rightSelector ? nextPage.querySelector(rightSelector) as HTMLElement : null;
            ...
            // Remove header/contact blocks from left column on subsequent pages
            this.stripLeftHeader(nextLeftCol);

            // Add top spacer to columns that have content on the new page
        ...
        const sections = Array.from(contentContainer.querySelectorAll(':scope > .section-block')) as HTMLElement[];
        ...
        // Remove intro/header blocks before the flowing section container on continuation pages.
        this.stripSingleColumnHeader(nextContentContainer);

        // Add top spacer so content doesn't start flush at the top on new pages
  ...
  private findSplitIndex(sections: HTMLElement[], pageRect: DOMRect, maxContentHeight: number): number {
  ...
  private applySplit(currentSections: HTMLElement[], nextContainer: HTMLElement, splitIndex: number) {
      const nextSections = Array.from(nextContainer.querySelectorAll(':scope > .section-block')) as HTMLElement[];

      if (splitIndex === -1) {
          // No split needed for this column: remove all sections from the next page to avoid duplicates
  ...
  private applyColumnSplit(
      ...
      if (splitIndex > 0) {
          ...
          if (currentSection && nextSection) {
              const didChunkSplit = this.splitSectionByChunks(currentSection, nextSection, pageRect, maxContentHeight);
              if (didChunkSplit) {
                  this.hideSectionTitle(nextSection);
                  // Keep sections before splitIndex on current page, remove them from next page.
                  for (let i = 0; i < splitIndex; i++) {
                      if (nextSections[i]) nextSections[i].remove();
                  }
                  // Keep sections after splitIndex on next page, remove them from current page.
          ...
          // Fallback: move the whole section if we couldn't split inside it.
          this.applySplit(sections, nextContainer, splitIndex);
          return true;
      }

      const didChunkSplit = this.splitSectionByChunks(sections[0], nextSections[0], pageRect, maxContentHeight);
      if (!didChunkSplit) {
          return false;
      }
      this.hideSectionTitle(nextSections[0]);

      // Remove sections after the first from the current page (they will remain on the next page).
  ...
  private splitSectionByChunks(
      ...
      if (chunks.length === 0 || chunks.length !== nextChunks.length) {
          console.log('PDF CHUNKS: no chunks or mismatch', { chunks: chunks.length, nextChunks: nextChunks.length });
          return false;
      }
      ...
      if (splitIndex > 0) {
          ...
          if (didSplitOverflowingChunk) {
              // Keep chunks before splitIndex on current page, remove from next page.
              for (let i = 0; i < splitIndex; i++) {
                  if (nextChunks[i]) nextChunks[i].remove();
              }
              // Keep split chunk on both pages, move only following chunks to next page.
      ...
      const didInnerSplit = this.splitInsideChunk(chunks[0], nextChunks[0], pageRect, maxContentHeight);
      if (!didInnerSplit) return false;

      // Remove top-level chunks after the first from the current section.
  ...
  private splitInsideChunk(
      ...
      // 1) First try splitting nested section-chunk blocks (existing behavior).
      ...
      // 2) Then try splitting inside rich text content (paragraphs/list items/words).
      const currentRichTexts = Array.from(currentChunk.querySelectorAll('.rich-text')) as HTMLElement[];
      ...
      if (currentRichTexts.length > 0 && currentRichTexts.length === nextRichTexts.length) {
          for (let i = 0; i < currentRichTexts.length; i++) {
              if (this.splitElementRecursively(currentRichTexts[i], nextRichTexts[i], pageRect, maxContentHeight)) {
                  return true;
              }
          }
      }

      // 3) Final fallback: split generic block content inside the chunk.
  ...
  private splitElementRecursively(
      ...
      if (currentChildren.length === 1 && nextChildren.length === 1 &&
  ...
  private getSplitCandidates(container: HTMLElement): HTMLElement[] {
      ...
      const listItems = Array.from(container.querySelectorAll(':scope > ul > li, :scope > ol > li')) as HTMLElement[];
      if (listItems.length > 1) return listItems;
  ...
  private getFallbackChunks(section: HTMLElement): HTMLElement[] {
      ...
      const listItems = Array.from(contentRoot.querySelectorAll(':scope > ul > li, :scope > ol > li')) as HTMLElement[];
      if (listItems.length > 1) return listItems;
  ...
  private hideSectionTitle(section: HTMLElement) {
      const heading = section.querySelector(':scope > h1, :scope > h2, :scope > h3, :scope > h4') as HTMLElement | null;
      if (!heading) return;
